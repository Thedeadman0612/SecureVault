...
| `app/main.py` | Added rotating file logger — `logs/app.log` (10 MB × 5 backups); configured at import time via `logging.basicConfig` with `RotatingFileHandler`; `logs/` added to `.gitignore`; DEBUG level in development, INFO in production; noisy third-party loggers (`uvicorn.access`, `watchfiles`) suppressed to WARNING; Phase 4 will replace with structured JSON logging and a dedicated security-audit stream |
