...
| `app/database/session.py` | SQLAlchemy engine, `SessionLocal`, `get_db` dependency; rollback on exception before close to prevent dirty connections returning to pool; **WAL mode hardening**: `@event.listens_for(engine, "connect")` pragma hook enables `journal_mode=WAL` and `synchronous=NORMAL` on every new SQLite connection — readers never block writers, writers never block readers; no schema migration needed |
...
| `app/tests/test_csp.py` | New — 25 tests in three classes: `TestCSPHeaderPresence` (6 tests — header present on GET, unauthenticated redirect, CSRF 403, unknown-path redirect; value matches `CSP_HEADER_VALUE` constant), `TestCSPDirectiveValues` (8 tests — `script-src` has no `unsafe-inline`/`unsafe-eval`, `frame-ancestors 'none'`, `form-action 'self'`, `object-src 'none'`, `base-uri 'self'`, `connect-src 'self'`; all `_CSP_DIRECTIVES` keys present in live header), `TestDefenceInDepthHeaders` (11 tests — `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, and `Strict-Transport-Security` verified on GET, 302 redirect, and CSRF 403; HSTS value asserted to contain `max-age` ≥ 1 year and `includeSubDomains`; imports `_HSTS_NAME/VALUE` constants) |
...
| `GET /health` | Health check endpoint — returns `{"status": "ok", "db": "ok"}` (checks DB connectivity with a lightweight query); exempted from `AuthGuard`; used by load balancers, uptime monitors (UptimeRobot / AWS Route 53 health checks), and CI smoke tests |
| `scripts/backup.sh` | Nightly SQLite backup to AWS S3 — `sqlite3 securevault.db ".backup /tmp/backup.db"` → `aws s3 cp /tmp/backup.db s3://your-bucket/backups/$(date +%Y%m%d).db`; cron job on EC2 (`0 2 * * *`); teaches `boto3`/AWS CLI, cron, and SQLite online backup API; retention policy (keep last 30 days) enforced via S3 Lifecycle rules |
...
>
> ⚠️ **Backup security:** The S3 bucket must be private (no public access), server-side encrypted (SSE-S3 or SSE-KMS), and access-restricted to the EC2 instance via an IAM role — never via hardcoded AWS credentials. The backup file contains encrypted vault data; even if the S3 bucket were exposed, the ciphertext is protected by AES-256-GCM. Document in `docs/deployment.md`.
